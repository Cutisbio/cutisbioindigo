...
| `messages/it.json` | 이탈리아어 |
| `messages/zh.json` | 중국어(간체) |
| `messages/tr.json` | 터키어 |

언어 선택기의 순서는 `src/data/blugene/site.ts` 의 `LOCALES` 순서를 그대로 따릅니다
(2026-09-11 고객 지정: 한국어 · 일본어 · 영어 · 프랑스어 · 이탈리아어 · 중국어 · 터키어. 벵골어는 같은 날 뺐고,
옛 `/bn` 주소는 영어 페이지로 넘어갑니다).
...
**한국어(`ko.json`)만 고치고 아래를 실행하면** 나머지 6개 언어가 채워집니다.
...
번역을 손으로 고쳐도 됩니다. 그때는 6개 파일의 같은 자리를 직접 고치면 됩니다.

> **방문자가 언어를 바꿀 때는 API 를 부르지 않습니다.** 7개 언어 페이지가 빌드 때 미리 만들어지고,
> 언어 전환은 이미 만들어진 페이지로 이동할 뿐입니다.

### 규칙 네 가지

1. **한국어(`ko.json`)를 먼저 고치고, `npm run translate` 를 실행하거나 나머지 6개 언어를 직접 고칩니다.**
...
4. `<link>…</link>`, `<keep>…</keep>` 처럼 **꺾쇠로 감싼 태그도 그대로 두세요.** `<link>` 는 링크가 걸리는 부분이고,
   `<keep>` 은 브랜드 페이지 제목(`Brand.heroTitle`)에서 뒷구절 「염료부터 꼼꼼히 확인해야 합니다.」 를
   태블릿 이상 화면에서 한 줄로 묶어 두는 표시입니다. 한국어에만 있으며 다른 언어에는 넣지 않아도 됩니다.
   제목을 바꿔서 한 줄로 묶을 구절이 달라지면 태그 위치만 옮기세요.
...
- **분류 배지**(보도자료 · 제품출시 · 업무협약 · 공동연구)는 번역시키지 않고
  ...
  (2026-09-11 까지 있던) 벵골어에서 세 가지로 갈렸습니다. 분류를 새로 만들면 이 표에도 6개 언어를 채워 넣으세요.
