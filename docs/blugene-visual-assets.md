...
## 2. 저장소에 원래 있던 자산 중 계속 쓰는 것

| 웹 경로 | 성격 | 용도 |
|---|---|---|
| `/logo.png` | 브랜드 이미지 | 회사 소개 페이지의 CutisBio 로고 |
...
| `Wordmark.tsx` | Blugene by CutisBio 워드마크. 서체는 `.blugene-wordmark`(globals.css) 한 곳에서 관리한다 | 브랜드 표현 |
...
C(정확한 SVG 개념도)와 D(근거를 읽는 차트·색상 도구)는 위 3절과 같이 **코드로 구현**했다.
